...
class DreamGenerationConfig(GenerationConfig):
    def __init__(self, **kwargs):
        ...
        self.eos_penalty: Optional[float] = kwargs.pop("eos_penalty", 0.0)
...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        result, used_steps = self._sample(
            input_ids,
            attention_mask=attention_mask,
            generation_config=generation_config,
    ...
    def _sample(
        ...
        while (x == mask_token_id).any():
            ...
            logits = self(x, attention_mask, tok_idx).logits  # [B, L, V]
            logits = torch.cat([logits[:, :1], logits[:, :-1]], dim=1)  # shift to align

            logits = generation_logits_hook_func(step_id, x, logits)

            mask_logits = logits[mask_index]  # [num_mask_positions, V]
            t = timesteps[step_id]
            #s = timesteps[step_id + 1]
            if mask_logits.numel() > 0:
                mask_logits = mask_logits.clone()
                # add to PAD token logit (shape check)
                if pad_token_id < mask_logits.shape[-1]:
            ...
            for b in range(B):
                ...
                if num_mask_pos > 0:
                    ...
                    # nothing to unmask in this batch
                    continue
            newly_unmasked = selected_mask & mask_index  
            new_unmask_counts = newly_unmasked.sum(dim=1)
            #print(f"step {step_id}: new_unmask_counts per batch = {new_unmask_counts.tolist()}")
            if newly_unmasked.any():
                x[newly_unmasked] = candidate_tokens[newly_unmasked]
                # record first-time confidence only if not recorded before (NaN)
            ...
            current_token_probs = probs.gather(-1, x.unsqueeze(-1)).squeeze(-1)  # [B, L], float
            # now per-batch choose lowest-prob tokens among current_unmasked_mask to remask
            ...
            for b in range(B):
                ...
                if cur_unm_count <= 1:
                    continue  # nothing or can't remask
                # desired number to remask
                num_remask = math.ceil(new_unmask_counts / 8)
                # clamp to [1, cur_unm_count - 1]
                num_remask = max(1, min(num_remask, new_unmask_counts - 1))
                # find lowest current-time confidences among cur_unm_pos
                cur_conf = current_token_probs[b, cur_unm_pos]  # shape [cur_unm_count]
            ...
            x = generation_tokens_hook_func(step_id, x, logits)
            step_id += 1
